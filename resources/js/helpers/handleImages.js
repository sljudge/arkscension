...
export default function handleImages(container, imgUrl, classes) {
    /**
    ...
    function createImgButton(container) {
        ...
        placeholder.className += "absolute top-0 right-0 img-placeholder z-20 bg-gray-200 cursor-pointer"

        const fileInput = document.createElement('input')
        fileInput.type = 'file'
        fileInput.className += "absolute top-0 right-0 w-20 h-20 z-30 opacity-0 cursor-pointer"
        ...
        if (container.style.backgroundImage) {
            ...
            placeholder.classList.add("hover:bg-red-700")
    ...
    ['dragenter', 'dragover', 'dragleave', 'drop'].forEach(eventName => container.addEventListener(eventName, (e) => preventDefaults, false))
    ...
    ['dragenter', 'dragover'].forEach(eventName => container.addEventListener(eventName, highlight, false))
    ...
    ['dragleave', 'drop'].forEach(eventName => container.addEventListener(eventName, unhighlight, false))
    ...
    function handleFiles(files) {
        ...
        reader.onloadstart = () => {
            ...
            placeholder.classList.add('bg-green-800')
        }
        //image loaded
        reader.onloadend = () => {
            container.style.backgroundImage = `url(${reader.result})` // update image
            placeholder.innerHTML = trashIcon // change to file icon
            placeholder.classList.add("hover:bg-red-800")
            placeholder.classList.remove('bg-green-800')
